...
DEFAULT_SETTINGS: dict[str, Any] = {
    ...
    # --- F1: гейт диаризации по длительности (спека 2026-08-12) ---
    # Диаризация (~1x realtime на MPS) на обычном пути диктовки
    # (recording_core_service._stop_recording_phase_c) не гейтилась по
    # длительности вовсе — короткая диктовка (40-60с) гоняла тот же полный
    # прогон, что часовая запись (живой инцидент: 44с диаризации на 42с
    # диктовке). Аудио короче порога не диаризуется, даже если
    # diarization_enabled=True. 0 (или отрицательное) полностью восстанавливает
