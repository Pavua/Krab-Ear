...
_BRAND_REPLACEMENTS_RAW: list[tuple[str, str]] = [
    ...
    (r"\bквен\b", "Qwen"),
    (r"\bКвен\b", "Qwen"),
    # "вен" (3 letters alone — НЕ как часть слова) когда рядом число/B/параметр —
    # это Whisper-shorthand для "Qwen". Пример live: "4-бит вен", "загрузится вен 14B".
    # Bind на context: за digit/B/parameter, чтобы не сломать "вена" (nominative).
    (r"\bвен\s+(?=\d+\s*[BbБб]?\b)", "Qwen "),
    (r"(?<=\d[\s\-])(?:бит|bit)\s+вен\b", "bit Qwen"),
    (r"\bпараметр\w*\s+вен\b", lambda m: m.group(0).replace("вен", "Qwen")),
    # GigaAM mishear
    ...
    (r"(?<=[.,!?])\s+0\s+(?=[А-Яа-яЁё])", " ОК, "),
    # "0" followed by typical Russian imperative/conversation continuation
    # (т.е. где «0» — это не число, а 100% разговорный маркер ОК):
    # давай(те), продолжаем, поехали, работаем, делаем, идём, начинаем,
    # пошли, понятно, хорошо, ладно, готово, надо, нужно, значит.
    # Сделано как одна строка — иначе lookahead не сработает.
    (
        r"\b0\s+(?=(?:[Дд]авай(?:те)?|[Пп]родолжа(?:ем|ю|й)|[Пп]оехали|"
        r"[Рр]аботае(?:м|шь|т)|[Рр]аботать|[Дд]ела(?:ем|ю|й|ть)|"
        r"[Сс]дела(?:ем|ю|й|ть)|[Нн]ачина(?:ем|й|ю)|[Нн]ачн[её]м|"
        r"[Ии]д[её]м|[Ии]дти|[Ии]ди|[Пп]ош[её]л|[Пп]ойд[её]м|"
        r"[Сс]лушай(?:те)?|[Сс]мотри(?:м|те)?|"
        r"[Пп]онятно|[Хх]орошо|[Лл]адно|[Гг]отов(?:о|а|ы)?|"
        r"[Нн]адо|[Нн]ужно|[Зз]начит)\b)",
        "ОК ",
    ),
    # Whisper artefact: digit "0" inside word in place of "и/о/у" letters.
    # Live cases: "0ли" → "или", "0х" → "их"/"ах" (твой/ты), "0бо" → "ибо".
    # Conservative: only when "0" stands at start of cyrillic letter cluster.
    (r"\b0ли\b", "или"),
    (r"\b0х(?=\s)", "их"),
    (r"\b0бо\b", "ибо"),
    # Whisper artefact: standalone "0" between two RU words = conjunction "и".
    # Live cases (2026-04-29 диктовка):
    #   "как я 0 говорил отлично" → "как я и говорил отлично"
    #   "вот 0 проскакивает" → "вот и проскакивает"
    #   "знаки препинания 0 грамматика" → "знаки препинания и грамматика"
    # ВАЖНО: Negative lookahead на measurement units (градусов/минут/etc.)
    # сохраняет численные контексты "температура 0 градусов".
    # Apply LAST after "ОК" patterns — уже обработали filler+0+verb cases.
    (
        r"(?<=[А-Яа-яЁё])\s+0\s+"
        r"(?![Гг]рад\w*|[Мм]инут\w*|[Сс]екунд\w*|[Чч]ас\w*|[Сс]ут\w*|"
        r"[Дд]не[йя]?\b|[Лл]ет\b|[Гг]од\w*|[Нн]ед[еь]\w*|[Мм]есяц\w*|"
        r"[Рр]убл\w*|[Дд]олл\w*|[Бб]алл\w*|[Шш]тук\w*|[Пп]роцент\w*|"
        r"[Гг]рамм(?:ов|а|у)?\b|[Кк]м\b|[Сс]м\b|[Мм](?=\W)|[Кк]г\b|"
        r"[Пп]ара|[Ши]аг\w*|[Рр]аз\b|[Вв]ольт|[Аа]мпер|[Гг][ББ]\b)"
        r"(?=[А-Яа-яЁё])",
        " и ",
    ),
    # Bonus typo from Whisper: "припинания" → "препинания"
    (r"\bприпинани\w+\b", lambda m: m.group(0).replace("припин", "препин")),
