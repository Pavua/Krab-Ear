...
_HALLUCINATION_PATTERNS: list[re.Pattern] = [
    re.compile(pat) for pat in [
        ...
        # W4: голая подпись без глагола (дыра W1894). Строго ПОСЛЕ субтитр-
        # паттерна: иначе «субтитры создавал <name>» срежет только токен,
        # оставив «субтитры создавал». Режем только хвост, не середину.
        r"(?:dima\s?torzok|дима\s?торзок|dimatorzok|диматорзок)[.!?…]*$",
