    ...
    def transcribe(
        ...
        try:
            ...
            return {
                ...
                # 🔴 Значение обязано ехать вместе с источником: без пометки оно
                # неотличимо от замера, и потребители (тренды качества,
                # предупреждения, гейт multipass-ретрая) считают фикцию фактом.
                # Замер 03.09.2026 на живой истории: GigaAM всегда 0.90 против
                # 0.66–0.74 у Whisper — русские записи выглядят лучше независимо
                # от реального качества, а ретрай по уверенности для них мёртв.
                "confidence": 0.9,
