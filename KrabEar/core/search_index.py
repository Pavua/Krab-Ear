...
class SearchIndex:
    ...
    # ------------------------------------------------------------------
    # Очистка (privacy-purge)
    # ------------------------------------------------------------------

    def clear(self) -> None:
        """Полностью сбрасывает индекс, тексты и сигнатуру.

        ``self._texts`` хранит полный cleartext-текст КАЖДОЙ записи истории
        (для генерации снипетов), поэтому при privacy-purge (wipe-all) этот
        in-RAM слепок ПДн обязан стираться вместе с диском. Сброс сигнатуры в
        ``None`` гарантирует, что следующий ``build_index`` перестроит индекс
        с нуля, а не примет пустое состояние за актуальное.
        """
        with self._lock:
            self._index = {}
            self._texts = {}
