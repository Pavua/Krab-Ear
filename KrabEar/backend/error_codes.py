...
ERROR_REGISTRY: dict[str, _Entry] = {
    ...
    # тут нечего, это сигнал «загляни в форензику».
    #
    # 🔴 dedupe_seconds ниже для ЭТОГО кода не работает и работать не может:
    # ErrorBus._last_emitted живёт в памяти процесса, а _report_unclean_restart
    # вызывается ровно ОДИН раз за жизнь процесса — следующий подъём начинает с
    # чистой памяти. Настоящий страж от шторма — кросс-рестартовый лимит на
    # диске, BackendService.UNCLEAN_RESTART_REPORT_MIN_GAP_SEC. Снимать его как
    # «избыточный», доверившись значению ниже, нельзя.
    #
