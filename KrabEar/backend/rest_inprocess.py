...
class InProcessRestServer:
    ...
    def __init__(
        ...
        # R2-фикс 1 (финальное adversarial-ревью волны S3): НЕОБРАТИМАЯ
        # защёлка — в отличие от ``_shutting_down`` (переиспользуемый:
        # start() штатно его сбрасывает после лечения сторожа), ``_terminal``
        # взводится ОДИН раз в begin_shutdown() и никогда не снимается за
        # жизнь этого экземпляра. Закрывает гонку: RestWatchdog вызывает
        # ``restart()`` (rest_inprocess.py, метод ниже), тот живёт до
        # RESTART_STOP_DEADLINE_SEC=6с на отдельном треде и внутри
        # БЕЗУСЛОВНО зовёт self.start() уже ПОСЛЕ своего stop(). Если SIGTERM
        # приходит в это окно, RestWatchdog.stop() (join(timeout=2.0) в
        # rest_watchdog.py) не дожидается идущего restart() — тред
        # "зомби"-restart() переживает join и вызывает self.start() уже
        # ПОСЛЕ того, как _shutdown_backend/close() продвинулись дальше
        # (rest_inprocess.stop(), release_external_singletons()), поднимая
        # Flask заново над полузакрытым backend'ом. Раньше терминальность
        # держалась только СНАРУЖИ (комментарий "start() уже никто не
        # позовёт" в start() ниже) — неверно именно для этого пути.
    ...
    def begin_shutdown(self) -> None:
        ...
        Идемпотентен (``threading.Event.set()``): повторный вызов не имеет
        дополнительного эффекта.

        R2-фикс 1: взводит ТАКЖЕ необратимую защёлку ``_terminal`` (см.
        docstring поля в __init__) — единственная линия обороны против
        "restart()-в-полёте", переживающая гонку с RestWatchdog.stop().
        Поэтому, в отличие от версии до фикса, ``stop()`` больше НЕ зовёт
        этот метод для своей внутренней гейтовки (см. docstring stop()) —
        только явный вызов снаружи (``_shutdown_backend``) взводит
        терминальность; сам ``begin_shutdown()`` остаётся идемпотентен и
        безопасен для повторного вызова оттуда же.
        """
    ...
    def start(self) -> bool:
        ...
        # R2-фикс 1: терминальная защёлка проверяется ПЕРВЫМ действием, ДО
        # сброса shutting_down. Раньше терминальность держалась только
        # СНАРУЖИ ("после начала останова процесса start() уже никто не
        # позовёт") — неверно для restart() (см. docstring ниже и
        # docstring self._terminal в __init__): тот зовёт self.start() на
        # отдельном треде уже ПОСЛЕ своего stop(), и это может произойти
        # ПОСЛЕ того, как _shutdown_backend/close() продвинулись дальше.
        # Без этой проверки — "зомби"-start() поднимает Flask заново над
        # уже закрывающимся/закрытым backend'ом.
        if self._terminal.is_set():
            logger.warning(
                "InProcessRestServer: start() отклонён — терминальная "
                "защёлка уже взведена (backend завершает работу)"
            )
            return False
    ...
    def stop(self, timeout: float = SHUTDOWN_JOIN_TIMEOUT_SEC) -> None:
        ...
        же действием — вызывается и из финального shutdown backend'а
        (обычно ПОСЛЕ отдельного раннего begin_shutdown() из
        _shutdown_backend — тут это лишь идемпотентный повтор), и напрямую
        ...
        R2-фикс 1: сознательно ставит ``_shutting_down`` НАПРЯМУЮ, а НЕ через
        ``begin_shutdown()`` — тот теперь взводит ТАКЖЕ необратимую
        ``_terminal`` (см. docstring begin_shutdown()/поля __init__), а
        ``stop()`` вызывается и из НЕтерминального пути (restart() =
        stop() + start(), лечение сторожа). Если бы stop() шёл через
        begin_shutdown(), самое первое лечение сторожа за жизнь процесса
        взводило бы _terminal и на этом же тике убивало бы восстановление
        REST навсегда — той же ценой, что и баг, который этот фикс закрывает.

        Терминальность (запрет вновь открыть допуск НАВСЕГДА) обеспечивается
        ИСКЛЮЧИТЕЛЬНО явным вызовом begin_shutdown() (со стороны
        _shutdown_backend) — не самим stop(): start() сбрасывает
        shutting_down безусловно (кроме терминальной защёлки), поэтому
        именно вызывающий решает, будет ли за этим stop() ещё один start().
        """
        self._shutting_down.set()
