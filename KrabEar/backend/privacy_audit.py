...
Thread safety (W1027 F1 HIGH — race fix):
- ``_log_lock`` (threading.Lock) сериализует весь цикл read→compute→write→update
  внутри процесса, предотвращая гонку двух потоков на ``_last_hash``.
- ``fcntl.flock`` остаётся для межпроцессной синхронизации на диске.
"""
...
class PrivacyAuditLogger:
    ...
    Thread safety (W1027 F1 HIGH):
        ``_log_lock`` сериализует весь цикл read→compute→write→update в
        ``log_event()``, предотвращая гонку на ``_last_hash`` при конкурентных
        вызовах из нескольких потоков.
    """
    ...
    def __init__(self, log_path: Path | None = None) -> None:
        ...
        # Инстанс-уровневый lock: сериализует read→compute→write→update в log_event().
        # Отдельен от fcntl.flock (межпроцессная синхронизация).
        # Fix: W1027 F1 HIGH — _last_hash race condition.
    ...
    def log_event(
        ...
        Весь цикл read(_last_hash)→compute(HMAC)→write(disk)→update(_last_hash)
        защищён ``_log_lock``, чтобы конкурентные потоки не могли прочитать
        одинаковый prev_hash и сгенерировать две записи с одинаковым звеном
        цепочки, ломая ``verify_chain()``. (W1027 F1 HIGH)
        ...
        """
        with self._log_lock:  # serialize entire chain-update cycle (W1027 F1 fix)
            ...
            # Хеш-цепочка: читаем prev_hash под lock
            ...
            try:
                ...
                # flock остаётся для межпроцессной синхронизации.
                with self._log_path.open("a", encoding="utf-8") as fh:
                    ...
                    try:
                        ...
                        fcntl.flock(fh.fileno(), fcntl.LOCK_UN)
                # Обновляем кончик цепочки после успешной записи (под _log_lock)
