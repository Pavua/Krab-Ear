...
_RANGE_FIELDS: dict[str, tuple[Any, Any, Any, type]] = {
    ...
    # settings-read-nonblocking). 0 разрешён явно (прежнее поведение —
    ...
    # 🔴 ВЕСТИГИАЛЬНО с волны ping-zero-wait (тот же день, тот же спека-файл):
    # живой замер показал, что сумма НЕСКОЛЬКИХ бюджетированных попыток на
    # одном пути ping (settings-read + count-read) доходит до 2.04с при
    # дефолтных значениях — Swift-таймаут ping ровно 2с. handle_ping() больше
    # НЕ ЧИТАЕТ это значение (StateStore.count_active_items(nowait=True) —
    # РОВНО одна неблокирующая попытка, независимо от бюджета). Поле
    # остаётся в схеме валидатора для обратной совместимости settings.json
    # (старые/сторонние клиенты могли выставить это значение) — просто больше
    # ни на что не влияет.
