...
class Transcriber:
    ...
    def transcribe_preview(self, audio_data: Any, quality_profile: str = "balanced") -> dict[str, Any]:
        ...
        # 🔴 Лок ОТПУСКАЕМ СРАЗУ — он был пробой занятости, а не защитой.
        # Удержание через весь transcribe() гарантированно блокировало сам
        # инференс: движок отправляет адаптер в ThreadPoolExecutor
        # (engine.py, `_pool.submit(adapter_fn)`), а адаптер берёт ТОТ ЖЕ
        # mlx_lock (stt_gigaam_mlx.py). RLock реентерабелен только для СВОЕГО
        # потока — рабочий поток пула ждал лок, удерживаемый тем, кто его
        # породил, и упирался в свои 25с. Живой лог 02.09.2026:
        #   GigaAM-MLX: mlx_lock занят дольше 25.0с — уступаю очередь
        # на КАЖДОЙ итерации превью, после чего «preview worker не завершился
        # за 1.5 с», а финальная транскрипция конкурировала с недобитым превью
        # и вылетала за бюджет (90с на 30с речи).
        #
        # Отпускать безопасно: КАЖДЫЙ downstream-путь берёт лок сам —
        # GigaAM-адаптер (stt_gigaam_mlx.py) и whisper (_transcribe_model,
        # `with mlx_inter_process_lock(), mlx_lock():`). Инвариант «любой
        # MLX-инференс под локом» не нарушается, дублирующий внешний захват
        # снимается.
        #
        # TOCTOU между release и захватом адаптера осознан: превью
        # косметическое, а у адаптера своё ограниченное ожидание. Проба
        # остаётся полезной — она отсеивает итерации, когда GPU занят прямо
        # сейчас, не платя за это блокировкой.
        lock.release()
