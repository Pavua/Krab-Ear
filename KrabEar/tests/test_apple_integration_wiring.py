"""Wiring guard tests for AppleIntegrationService — Wave 752.
...
class TestAppleIntegrationWiring(unittest.TestCase):
    """Confirm AppleIntegrationService is imported, instantiated, and all
    ...
    def test_module_imported(self) -> None:
        self.assertIn(
            ...
            self.source,
            'AppleIntegrationService import missing from service.py',
        )
    ...
    def test_svc_instantiated_in_init(self) -> None:
        pattern = re.compile(
            r'self\._apple_integration_svc\s*=\s*AppleIntegrationService\(',
        )
        self.assertRegex(
            self.source,
            pattern,
            'self._apple_integration_svc = AppleIntegrationService( not found in service.py',
        )
    ...
    def test_handlers_delegate_to_svc(self) -> None:
        for handler_method in _HANDLER_NAMES:
            ...
            stub_pattern = re.compile(
                ...
                rf'(?:\s*"""[^"]*"""\s*\n)?'
                rf'\s*return\s+self\._apple_integration_svc\.{re.escape(handler_method)}\(',
                re.DOTALL,
            )
            direct_pattern = re.compile(
                rf'self\._apple_integration_svc\.{re.escape(handler_method)}',
            )
            found = (
            ...
            self.assertTrue(
                found,
                msg=(
                    f'{handler_method} is not wired to _apple_integration_svc '
