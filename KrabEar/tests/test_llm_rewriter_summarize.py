...
class SummarizeEmptyInputTestCase(unittest.TestCase):
    ...
    def test_empty_string_returns_empty_input(self, mock_post):
    ...
    @patch("backend.llm_rewriter.requests.post")
    def test_whitespace_only_returns_empty_input(self, mock_post):
        result = self.rewriter.summarize("   \n  \t")
        self.assertFalse(result.ok)
        self.assertEqual(result.fallback_reason, "empty_input")
        mock_post.assert_not_called()
...
class SummarizeSuccessTestCase(unittest.TestCase):
    ...
    @patch("backend.llm_rewriter.requests.post")
    def test_successful_summary_returns_ok(self, mock_post):
        mock_post.return_value = _mock_ok_response("Краткое резюме текста.")
    ...
    @patch("backend.llm_rewriter.requests.post")
    def test_summary_has_latency_ms(self, mock_post):
        mock_post.return_value = _mock_ok_response("Summary.")
        result = self.rewriter.summarize("Some text to summarize.")
        self.assertIsNotNone(result.latency_ms)
        self.assertGreaterEqual(result.latency_ms, 0)

    @patch("backend.llm_rewriter.requests.post")
    def test_summary_uses_post_to_chat_completions(self, mock_post):
        mock_post.return_value = _mock_ok_response("Summary.")
    ...
    @patch("backend.llm_rewriter.requests.post")
    def test_summary_sends_authorization_header(self, mock_post):
        mock_post.return_value = _mock_ok_response("Summary.")
    ...
    @patch("backend.llm_rewriter.requests.post")
    def test_summary_uses_max_sentences_in_system_prompt(self, mock_post):
        mock_post.return_value = _mock_ok_response("Summary.")
    ...
    @patch("backend.llm_rewriter.requests.post")
    def test_summary_default_max_sentences_3(self, mock_post):
        mock_post.return_value = _mock_ok_response("Summary.")
        self.rewriter.summarize("Text.")
        _, kwargs = mock_post.call_args
        messages = kwargs["json"]["messages"]
    ...
    @patch("backend.llm_rewriter.requests.post")
    def test_summary_timeout_is_doubled(self, mock_post):
...
class SummarizeCircuitBreakerTestCase(unittest.TestCase):
    ...
    @patch("backend.llm_rewriter.requests.post")
    def test_circuit_opens_after_3_failures(self, mock_post):
        import requests as req
    ...
    @patch("backend.llm_rewriter.requests.post")
    def test_circuit_open_blocks_summarize(self, mock_post):
