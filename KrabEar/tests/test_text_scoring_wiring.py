"""Wiring guard tests for TextScoringService — Wave 752.
...
class TestTextScoringWiring(unittest.TestCase):
    """Confirm TextScoringService is imported, instantiated, and all
    3 handlers are delegated in service.py."""
    ...
    def test_module_imported(self) -> None:
        self.assertIn(
            ...
            self.source,
            'TextScoringService import missing from service.py',
        )
    ...
    def test_svc_instantiated_in_init(self) -> None:
        pattern = re.compile(
            r'self\._text_scoring_svc\s*=\s*TextScoringService\(',
        )
        self.assertRegex(
            self.source,
            pattern,
            'self._text_scoring_svc = TextScoringService( not found in service.py',
        )
    ...
    def test_handlers_delegate_to_svc(self) -> None:
        for handler_method in _HANDLER_NAMES:
            ...
            stub_pattern = re.compile(
                ...
                rf'(?:\s*"""[^"]*"""\s*\n)?'
                rf'\s*return\s+self\._text_scoring_svc\.{re.escape(handler_method)}\(',
                re.DOTALL,
            )
            direct_pattern = re.compile(
                rf'self\._text_scoring_svc\.{re.escape(handler_method)}',
            )
            found = (
            ...
            self.assertTrue(
                found,
                msg=(
                    f'{handler_method} is not wired to _text_scoring_svc '
