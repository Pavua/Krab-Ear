...
def test_telnyx_429_sleep_capped_at_max() -> None:
    ...
    mock_resp.headers = {"Retry-After": "120"}  # сервер говорит подождать 120s

    sleep_calls: list[float] = []

    with patch("backend.telnyx_adapter.time.sleep", side_effect=lambda s: sleep_calls.append(s)):
        result = adapter._handle_response(mock_resp)

    assert result["ok"] is False
    assert result["error"] == "rate_limit"
    # Должен быть ровно один вызов sleep
    assert len(sleep_calls) == 1, f"Ожидался 1 sleep-вызов, получили {sleep_calls}"
    # И этот sleep не должен превышать потолок
    assert sleep_calls[0] <= ta._RETRY_AFTER_MAX_SEC, (
        f"sleep({sleep_calls[0]}) превышает потолок {ta._RETRY_AFTER_MAX_SEC}"
    )


def test_telnyx_429_no_sleep_in_stub_mode() -> None:
    """В stub-режиме (нет api_key) _handle_response никогда не вызывается — проверяем что
    адаптер возвращает not_configured без sleep."""
    import backend.telnyx_adapter as ta

    adapter = ta.TelnyxAdapter()  # stub mode
    with patch("backend.telnyx_adapter.time.sleep") as mock_sleep:
        result = adapter.dial("+79001234567")
    assert result["ok"] is False
    assert result["error"] == "telnyx_not_configured"
...
def test_twilio_429_sleep_capped_at_max() -> None:
    ...
    adapter = ta.TwilioAdapter(account_sid=account_sid, auth_token="tok", from_number="+10000000000")
    ...
    with patch("backend.twilio_adapter.time.sleep", side_effect=lambda s: sleep_calls.append(s)):
        result = adapter._handle_response(mock_resp)
