...
- **`backend/service.py`** — `BackendService` (business logic) + `IPCServer` (Unix socket server). Single file, **~3448 lines** (down from 5821 baseline, −41% after marathon extractions). The `handle_request` method dispatches **304** JSON-RPC methods via a handler lookup table, delegating to **13 extracted services** (Wave 65 batch 1 removed 19 dead handlers; marathon waves W172/W392/W404/W423/W525/W683/W691/W734/W741/W742 added RecordingCoreService, AnalyticsService, TextScoringService, HealthCheckService, AppleIntegrationService [wired W741], STTManagementService [wired W742], and others). Post-W742 the lookup table may bypass local `_handle_X` shims and call `self._<svc>.handle_X` directly for already-extracted handlers. Full API reference: `docs/IPC_API_REFERENCE.md` (PR #243, 4341 lines) — has 58% drift per W657 audit; cross-check live via `grep -cE '"[a-z_]+":\s*self\._' KrabEar/backend/service.py`.
