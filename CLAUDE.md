...
GigaAM v3 (RU STT, 2026-07-20): `STT_GIGAAM_MODE` дефолт `v3_e2e_rnnt` (был `rnnt`→v2). v3-модели ТОЛЬКО в git-исходнике (`salute-developers/GigaAM`, пиновано `559d88d6` в `scripts/install_gigaam_venv.command`), НЕ в PyPI `gigaam==0.1.0` (v1/v2). 🔴 В git-пакете голое `"rnnt"` алиасится в `v3_rnnt` (не v2_rnnt) → указываем полное имя. v3_e2e даёт нативную пунктуацию/капитализацию/числа (v2 — всё строчными); живой бенч M4 Max: не медленнее v2, на технике быстрее. torch-пин ослаблен `>=2.6`. Longform (>30с, звонки/импорт) требует отдельного `[longform]` extra (torch 2.10 + torchcodec + pyannote 4) — для диктовки не триггерится, возможный хвост. `PunctuationFixer` идемпотентен (двойной пунктуации нет), но его RU casing-правила (я→Я) писались под lowercase-вход v2 → лёгкая over-correction на cased-выводе v3, маскируется LLM-полировкой (известный косметический хвост). Спека `docs/superpowers/specs/2026-07-20-gigaam-v3-upgrade-design.md`.
