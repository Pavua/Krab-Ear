...
- **S34 clipboard safety (2026-07-19)**: `PasteService.putToClipboard(_:)` — единая точка защиты (все 7 call sites проходят через неё) — не затирает системный буфер, если он помечен `org.nspasteboard.ConcealedType` (де-факто стандарт nspasteboard.org, менеджеры паролей 1Password/Bitwarden/Keychain Access так помечают чувствительный контент); при пропуске зовёт `onConcealedClipboardSkipped` closure (тот же паттерн, что `onSecureFieldSkipped` для секретных ПОЛЕЙ-назначения). `main.swift` подключает прямой `notify()` — **НЕ** через `handlePasteFailure` (та сама зовёт `putToClipboard`, реентерабельность). Risk-warning tooltip у `clipboardModeSelector` (`HistoryPanelController.swift`) и у пунктов подменю «Буфер обмена» (`main+StatusMenu.swift`). Осознанно НЕ save/restore-таймер (сломал бы фичу «текст ждёт в буфере до ручной вставки») — только guard перед перезаписью.
