...
- **`HealthMonitor.swift`** — actor с 3s ping (IPC-метод ровно `"ping"`, НЕ `handle_ping` — так зовётся лишь внутренний Python-хендлер); 2 fails подряд → `state = .hung` → **однократный** `onHangDetected` → `BackendSupervisor.restartIfDeadDetailed()`. Сигналы отсюда НЕ шлются (раньше эта строка описывала несуществующий SIGTERM→SIGKILL→respawn и в разборе инцидента 2026-08-07 увела диагностику не туда). 🔴 Известный гэп той же даты: `hangFiredForCurrentEpisode` сбрасывается ТОЛЬКО успешным пингом, а `restartIfDeadDetailed()` на живом-но-заклиненном процессе возвращает `.alreadyAlive` и ничего не делает — значит на wedged-бэкенде выполняется ровно одна безрезультатная попытка, после чего self-heal молчит навсегда (класс «sticky state without an exit»). Лечение — эскалация к `forceRestartBackend()` под `WedgedEscalationTracker`, отличая отказ соединения (`IPCError.socketConnectFailed` = завис) от `.timeout` (может быть просто медленный бэкенд под нагрузкой — рестартить его нельзя, потеряется диктовка). Phase B.1 расширит подпиской на `rewriter_recovered` события из active LLM probe.
