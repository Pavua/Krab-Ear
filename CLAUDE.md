...
18. **MeetingSessionService** — живая meeting-сессия (C2 Live Meeting Overlay, 2026-07-11/16, релиз v2.9.0): GPU-слот-воркер (CHUNK_STT 25с / ITEMS_LLM 60с адаптивно / DIAR_WINDOW 90с за рубильником `meeting_live_speakers_enabled`), аккумулятор транскрипта по курсору `snapshot_range`, LiveSpeakerTracker (cosine-сшивка центроидов из `DiarizeOutput.speaker_embeddings` — НЕ gated `pyannote/embedding`; cap 16), brain-lease на встречу, privacy-гейты; IPC `meeting_start`/`meeting_stop`/`get_meeting_live_state`, события `meeting.*` (5). Swift: `MeetingLivePanelController` (SSE `/v1/events` + poll-фоллбэк, работает в ЛЮБОМ состоянии панели; one-shot гард `deliverFinished` — SSE finished и IPC-ответ оба несут item_id) + `main+MeetingPanel.swift` (меню-бар «Встреча») + standalone-окно `MeetingReportViewController`. Спека: `docs/superpowers/specs/2026-07-10-c2-live-meeting-overlay-design.md` (+§2.5a/§2.7a амендменты).
...
Fake-StateStore signature-drift guard: `scripts/audit_fake_store_signatures.py --fail-on-found` runs in CI (enforced since #1916 follow-up) — тесты подделывают `StateStore` вручную (свой класс или MagicMock-фабрика), реальная сигнатура уезжает вперёд, и фейки молча расходятся. 🔴 **Класс отложенный**: разошедшийся фейк остаётся ЗЕЛЁНЫМ, пока путь исполнения внутри теста не дойдёт до вызова с новым кваргом — поэтому волна «Контенция общего лока» (`2f27c547`, добавила `load_settings(lock_timeout_sec=, nowait=)`) уронила 7 файлов не сразу, а случайно, при постороннем PR, тогда как ещё **22** фейка стояли взведёнными и невидимыми. 🔴 **Критерий гарда — самое важное в нём**: фейк обязан принимать не всю сигнатуру реального класса, а ровно те **именованные** аргументы, которыми его реально зовёт прод (`store.load_settings(nowait=True)` → обязателен `nowait`; `store.save_settings(payload)` позиционно → требований нет; `**kwargs` → претензий нет). Наивный критерий «повторить сигнатуру целиком» был отвергнут замером — дал 21 ложную находку из 43 на `save_settings`, где фейки просто называют параметр `settings` вместо `new_settings`, а зовут его позиционно; гард с таким шумом не доживает до второго использования. 🔴 **Две оси поиска** (вторая добавлена 2026-08-23 после красного CI 22-08): (1) `def` с именем метода стора внутри фейк-класса/фабрики; (2) **подмена атрибута** — `<стор>.<метод> = функция|lambda` и сиблинг `setattr(стор, "метод", fn)`/`monkeypatch.setattr`. Слепой была именно вторая: `self.service.store.load_settings = tracked_load_settings` промахивалось мимо ОБОИХ условий первой оси — имя функции другое, а объемлющая область — обычный `test_*`-метод, не фейк-класс; контракт там устанавливает не объявление, а присвоение. Правая часть разрешается статически: `lambda` — по месту, голое имя — до `def` в ближайшей объемлющей области, а вызов (`MagicMock(...)`) и неразрешимое имя (`store.load_settings = original_load` — сохранённый оригинал) пропускаются: сигнатуры нет, требовать нечего. Расширение вскрыло вторую живую мину — `broken_load()` в `test_purge_privacy_gaps_w1767.py`, взведённую тем опаснее, что тест ЖДЁТ исключения и будущий `TypeError` проглотил бы молча, оставшись зелёным на другом отказе. Читает сигнатуры AST-разбором `state_store.py` БЕЗ импорта проекта (как соседние аудиты — быстрый CI-джоб без установленных зависимостей backend'а). Тоже `make audit-fake-store-signatures` (входит в `make audit-all`); `--selftest` гоняет известно-плохие/известно-хорошие образцы классификатора. Подключён ДВАЖДЫ, как и соседние гарды: шагом внутри `krabear-ci.yml backend-tests` (гейт на ubuntu) и отдельным job'ом `fake-store-signature-guard` в `ci.yml` — только второй виден как именованный чек в `gh pr checks`, падает за секунды и сразу называет проблему, вместо того чтобы прятаться в 20-минутном логе backend-tests. Job первым шагом гоняет `--selftest`: гард, тихо переставший классифицировать, отчитывался бы CLEAN вечно (зеркало правила «всегда красный монитор = слепота»).
GigaAM v3 (RU STT, 2026-07-20): `STT_GIGAAM_MODE` дефолт `v3_e2e_rnnt` (был `rnnt`→v2). v3-модели ТОЛЬКО в git-исходнике (`salute-developers/GigaAM`, пиновано `559d88d6` в `scripts/install_gigaam_venv.command`), НЕ в PyPI `gigaam==0.1.0` (v1/v2). 🔴 В git-пакете голое `"rnnt"` алиасится в `v3_rnnt` (не v2_rnnt) → указываем полное имя. v3_e2e даёт нативную пунктуацию/капитализацию/числа (v2 — всё строчными); живой бенч M4 Max: не медленнее v2, на технике быстрее. Точный upstream shortform limit — **25 секунд**, поэтому всё `>25.0s` обязано идти через `AudioChunker`; старый порог 30с терял диапазон 25–30с. v3 longform возвращает dataclass `LongformTranscriptionResult.segments`, legacy v1/v2 — `list[dict]`; оба формата нормализует `core/gigaam_compat.py`. One-click installer ставит `[longform]` (torchcodec + pyannote) сразу; основной chunker-путь HF-модели не требует. 🔴 stdout worker зарезервирован только под однострочный JSON: vendor progress из GigaAM/pyannote всегда перенаправлять в дренируемый stderr, иначе `_send()` прочитает progress вместо ответа. `PunctuationFixer` больше не превращает местоимение «я» внутри предложения в «Я»; капитализация начала предложения сохранена. Спека `docs/superpowers/specs/2026-07-20-gigaam-v3-upgrade-design.md`.
mlx-masking CI trap (🔴 recurring root cause): the dev `.venv_krab_ear` is Python 3.14 **with** mlx-whisper installed (macOS wheels exist); ubuntu `krabear-ci.yml` runs Python 3.12 with **no** mlx wheels. Any test asserting `import mlx_whisper` succeeds (or the STT-available branch) is a **false green** locally and fails on ubuntu — caused three red tips. Before merging, validate changed test files with the ubuntu-parity harness: `make pre-merge-check` (or `scripts/pre_merge_py312_check.sh <files>`, #1682) — builds/reuses a py3.12 venv with mlx purged, runs each file memory-safe. Ubuntu `krabear-ci.yml backend-tests` remains the per-PR Python gate. Public PR/push Swift jobs use disposable `macos-latest`; their public workflow must contain no self-hosted job. MLX/Metal coverage runs only in private `Pavua/Krab-CI-Control` on a trusted exact SHA after operational cutover. 🔴 Two measured constraints pin that design: (a) GitHub-hosted `macos-latest` CANNOT substitute — it is a VM without full Metal and mlx tests die there with `Abort trap: 6` (SIGABRT) while the same files are green on self-hosted; (b) the self-hosted runner cannot hold a per-PR gate on the owner's machine — under load it loses its control TLS connection to GitHub and the job fails with NO logs and NO steps (`SocketException (89): Operation canceled` in `~/actions-runner-krab-ear/_diag/Runner_*.log`; four drops on 01.09 alone, load average reached 278). A red private device gate is a signal to investigate, not a merge blocker.
...
Wake-word hard-negative training — TTS-эхо ≠ клавиатура (🔴 2026-07-12/13, T5b): дообучение `krab_ru` (openWakeWord) с hard-негативами реальной печати владельца (62→5 ложных срабатываний за 10 мин — почти решено) ОДНОВРЕМЕННО ухудшило устойчивость к СОБСТВЕННОМУ TTS через колонки (0→117 ложных срабатываний за 10 мин на `say -v Milena`) — три независимых прогона (два состава batch-composition, разные random seed) сошлись в один и тот же потолок, дальше крутить веса бессмысленно. Гипотеза (не окончательно доказана, но объясняет паттерн): весь позитивный корпус v1/v2 — 100% синтетический (Silero TTS), маленькая DNN (128 нейронов) частично выучивает «звучит как машинный синтез речи» как proxy-признак вместо чистой фонетики — отсюда всплеск именно на TTS-hard-негативах (тоже синтетическая речь), а не на не-речевом шуме (клавиатура). **Rule**: hard-negative retraining — не универсальный фикс на «ложные срабатывания wake word»; для конкретно TTS-self-echo (приложение само проигрывает голос рядом с своим же микрофоном) архитектурный фикс дешевле и надёжнее модели — приостанавливать wake-word слушатель на время собственного TTS-проигрывания (тот же паттерн, что `RealtimePartialTranscriber.pause()`/`SystemAudioDuckingService`). **РЕАЛИЗОВАНО 2026-07-13, PR #1876**: `WakeWordPauseReason.ttsPlayback` + пауза, завязанная на РЕАЛЬНЫЙ конец воспроизведения через `AVAudioPlayerDelegate.audioPlayerDidFinishPlaying` (НЕ на границы conversation-сессии — реальный корень был в том, что `ConversationErrorAnnouncer` озвучивает ошибку уже ПОСЛЕ `stopConversation()`, когда `.conversation`-пауза синхронно снята, а `AVAudioPlayer.play()` стартует асинхронно после round-trip `synthesize_speech` IPC). Перекрытие плеера и `play()==false` тоже ЯВНО постят `.krabTTSPlaybackFinished`, иначе пауза зависнет навсегда. Направление отказа fail-safe (не пришёл делегат → микрофон молчит дольше, а не ложно триггерится). Три раунда fine-tune (batch 512/256/256→640/384/128→768/256/128) — recall плато 0.70-0.714, TTS-эхо неизменно 117 детекций (PR #1875, не мержится, `hey_jarvis` остаётся прод-дефолтом). **Методологическая ловушка эвалюатора**: офлайн-скрипт с одним `Model()` на короткий (<3с) позитивный клип даёт заниженный recall (20% вместо честных 70% по внутреннему гейту auto_train) — скользящий буфер фичей openWakeWord не успевает прогреться за 5-40 чанков; такой харнесс достоверен ТОЛЬКО на длинных непрерывных записях (сотни-тысячи чанков, буфер прогрет) для подсчёта fp, не для recall на коротких клипах.
Wake-word `_listen_loop` — независимый от AudioSelfHealer класс залипания микрофона (🔴 живой инцидент 2026-07-13): владелец сообщил «ни "Краб", ни "Hey Jarvis" не срабатывают, индикатор микрофона в menu bar не загорается». `wake_word_status` при этом стабильно врал `running: true` (это ЖИВАЯ проверка `thread.is_alive()` — тред был жив) и `engine_available: true`; `test_microphone` (независимый, СВОЙ вызов `sd.InputStream`) отдавал нормальный RMS — общий аудио-стек ОС не мёртв. Sentry (`KRAB-EAR-BACKEND-1J`, `PortAudioError: Error opening InputStream [PaErrorCode -9986]`, culprit `openwakeword_adapter._listen_loop`) показал **4747** исторических срабатываний, но 0 новых в err.log текущего 17ч+ аптайма — то есть это не крэш-с-исключением, а тихое зависание ДО реального открытия CoreAudio: тред жив, `stream.read()` (или сам `with sd.InputStream(...)`) висит без исключения и без кадров, микрофон физически не активируется. **`OpenWakeWordAdapter.start()` — fire-and-forget**: спавнит `Thread(target=self._listen_loop, daemon=True)` и возвращает `ok:true` сразу, не дожидаясь фактического открытия потока — поэтому IPC-ответ выглядит здоровым, даже когда захват никогда не начинался. Рестарт ТОЛЬКО нити (`wake_word_stop`+`wake_word_start`) НЕ лечит — вопрос ниже уровня adapter'а, в состоянии самого процесса (тот же класс, что вчерашний мик-инцидент, который чинит `AudioSelfHealer`, но **AudioSelfHealer следит только за пайплайном диктовки/записи, НЕ за независимым потоком wake-word** — реальный пробел покрытия). Лечит ТОЛЬКО полный рестарт backend-процесса (`launchctl kickstart -k ai.krab.ear.backend`) — после него Swift-сторонний `WakeWordPoller` сам самовосстанавливает `wake_word_start` в течение одного цикла поллинга (0.75с), без ручного вмешательства. **Rule**: любой `X.is_running()`/`X_status` для фонового потока, спавнимого через голый `Thread.start()` без barrier/future на первый успешный цикл — недостоверен как сигнал «реально работает»; для долгоживущих аудио-потоков нужен heartbeat (таймстемп последнего успешно прочитанного чанка, не только «поток жив») с собственным watchdog-порогом, симметричным `AudioSelfHealer`. **ПОЧИНЕНО 2026-07-15 волной wake-word watchdog (PR #1879, Fable-архитектура)**: heartbeat `last_chunk_ts` (только ненулевые чанки — ловит и зависшее чтение 13-07, и шторм нулей 12-07) + generation-токен против зомби-тредов + `stop() -> bool` (THREAD_HUNG-дискриминатор: тред не вышел из join → `Pa_Terminate` запрещён) + maintenance-окно (чужой `wake_word_start` отвергается на участке stop→reinit — гонка с поллер-self-heal = crash-класс) в `openwakeword_adapter.py`; `backend/audio_reinit.py::AudioReinitCoordinator` — single-flight владелец танца reinit, общий для `AudioSelfHealer` (пассивный триггер) и нового `backend/wake_word_watchdog.py::WakeWordWatchdog` (активный: тик 5с, staleness ≥ `wake_word_stale_sec` (30с) → heal → эскалация `wedged:true` в `wake_word_status` + ErrorBus `audio.wakeword_wedged`; эпизоды, анти-шторм 3 танца/600с ВКЛЮЧАЯ THREAD_HUNG, dead-session аномалия для упавшего restore); Swift `WedgedEscalationTracker` (rate-limit 30 мин + give-up cap 3 подряд-эскалаций без здорового сигнала `last_chunk_ts` — restart-immune микрофон (громкость 0/TCC-нули) не даёт вечную карусель рестартов) → `BackendSupervisor.forceRestartBackend()` (`launchctl kickstart -k` в passive; НЕ `restartIfDead()` — тот short-circuit'ится на живом процессе); self-heal поллера подавлен при `wedged` (`!running && !wedged`), `resume(force:true)` — намеренный байпас для пере-эскалации. Спека: `docs/superpowers/specs/2026-07-15-wake-word-watchdog-design.md`. Живая проверка цепочки: заглушенный heartbeat на dev-инстансе → wedged:true за 25с. Секция `get_diagnostics.wake_word_watchdog` — live-диагностика. **🔴 Амендмент 2026-07-16 (авто-смок первой длинной встречи C2, `d18c5e56`):** meeting-запись (в отличие от хоткейной диктовки, которую WakeWordPoller снимает Set-причиной .recording) НЕ снимает wake-word слушатель → адаптер легитимно голодает по чанкам всю запись → staleness-шторм → THREAD_HUNG → wedged → агент kickstart'ил backend ПОСРЕДИ финальной транскрипции встречи (item потерян). Фикс: `WakeWordWatchdog` получает `is_recording` (тот же колбэк, что `AudioReinitCoordinator`) и трактует активную запись как ЛЕГИТИМНУЮ ПАУЗУ (эпизод/аномалия сбрасываются, ни heal, ни эскалации; is_recording() упал → fail-open в сторону работающего watchdog'а). **Rule**: первая ДЛИННАЯ запись новой фичи — обязательный смок-кейс для ВСЕХ фоновых сторожей (watchdog/selfheal) — они калибровались на коротких диктовках; любой сторож аудио-стека обязан знать про все источники легитимного голодания своего сигнала (запись, разговор, TTS, privacy), а не только про те, что снимают слушатель сами.
...
Контенция общего лока: чтение настроек и health-проба (волна стабильности 2026-08-12) — 🔴 `StateStore.load_settings()` берёт `self._lock()`, а это `history.lock` (`state_store.py:100`), тот же эксклюзивный flock, что охраняет ВСЮ историю. Privacy-гейт обязателен почти в каждом IPC-хендлере и читает настройки через `cached_settings()` → на промахе 5с-TTL встаёт в очередь за историей. Живой инцидент: MLX OOM → `MLX watchdog timeout 120s` → `_load_active_items_with_lock` держит flock → 20+ `load_settings ждёт эксклюзивный flock` → 11× `handle_request завис дольше 180с` → агент форс-рестартнул backend ПОСРЕДИ диктовки владельца (запись потеряна). Два фикса, оба задеплоены: (1) `settings_read_lock_timeout_sec` (дефолт 0.5) — `cached_settings()` на промахе TTL ждёт лок не дольше бюджета, затем отдаёт последнее известное значение; 🔴 направление отказа fail-closed по приватности — нет кэша вовсе → дефолты с ПРИНУДИТЕЛЬНЫМ `privacy_mode_enabled=True` (иначе транскрипты потекут в аналитику под контенцией). (2) `StateStore._lock(nowait=True)` — РОВНО ОДНА неблокирующая попытка `flock(LOCK_EX|LOCK_NB)` без цикла опроса, проброшена в `count_active_items(nowait=)`/`load_settings(nowait=)`/`SettingsService.cached_settings(nowait=True)`; `HealthCheckService._is_privacy_mode_nowait()` используется ТОЛЬКО в `handle_ping` (`get_diagnostics` осознанно остался на бюджетном пути). 🔴 **Правило для health-проб**: Swift `HealthMonitor` (`main+HealthMonitor.swift`) пингует с таймаутом **ровно 2.00с**, 2 промаха → `forceRestartBackend()`; проба, делящая блокирующий ресурс с рабочей нагрузкой, под контенцией даёт ложный вердикт «завис» и убивает ЗДОРОВЫЙ бэкенд. Короткие бюджеты этого НЕ решают — задержка равна СУММЕ бюджетов по пути (замер на проде: 0.5+0.3 → 2.04с не проходит; 0.05+0.05 → 0.12с), а число ждущих мест растёт от любой правки. Только zero-wait. Замер на проде владельца (12 591 запись, лок удерживается извне): было 2.04с → стало **0.55с** при тех же дефолтах. 🔴 Любой замер latency под локом гнать на РЕАЛЬНОМ объёме истории — воркер отчитался «0.32с» на store из одной записи, на проде вышло 2.04с. `ping_count_lock_timeout_sec` остался в `DEFAULT_SETTINGS`/`_RANGE_FIELDS` для совместимости settings.json, но `handle_ping` его больше не читает (вестигиальный). Разделение `settings.lock`/`history.lock` НЕ сделано: `migrate_history_encryption` берёт историю ПЕРЕД настройками → отдельный лок = lock-ordering дедлок; нужна волна с аудитом ~50 call sites.
...
- **`backend/recorder.py`** — `AudioRecorder`: thread-safe start/stop audio capture via `sounddevice`. R1 (2026-07-24/25): optional `RecordingSpillWriter` passed into `start(spill=...)` — worker appends raw PCM chunks strictly outside `_lock` (I/O-under-lock is the forbidden W1652/F3 class); `close()` called on every exit path except the `AudioRecorderStopTimeout` raise (worker may still be writing).
- **Владение записью (R2, 2026-07-25/28)** — общий `AudioRecorder` больше не «ничей»: `RecordingCoreService` заводит **поколение записи** (opaque `generation_token` + `owner` из allowlist `dictation`/`meeting`/`quick_capture` + `owner_revision` как CAS-lease), и `stop_recording` проходит гейт токенных инвариантов ПЕРВОЙ операцией под lock — mismatched stop не трогает preview/RT/RSF чужой записи. Ключевые статусы старта: `recording` (успех), `already_recording` (промоут встречи поверх идущей диктовки), `owner_conflict`, `unmanaged_recording` (рекордер занят захватом мимо Core — по сути Call Assist), `backend_closing`. 🔴 Статуса `"started"` НЕ существует — заглушка, возвращающая его, ловит строгую валидацию `MeetingSessionService`. Статусы стопа: `unknown_generation` (авторитетное «поколения нет»), `stop_in_progress`, `owner_mismatch`, `finalization_failed`, `recorder_stopping`. Терминальные ответы кэшируются с TTL и реплеятся по токену (двойной stop даёт ТОТ ЖЕ `history_id`, без второй финализации); кэш инвалидируется purge-эпохой и гейтится privacy. `generation_token` = `session_id` в spill-meta (R1-crash-safety не расходится с владением). Swift: `RecordingStopCoordinator` — единый helper остановки с bounded-ретраем; 🔴 `unknown_generation` обязан ЧИСТИТЬ локальный route (токен/`isRecording`/ducking), а не удерживать его как recovery-handle — иначе short-circuit в `handleRecordToggleRequest` (он намеренно не сверяется с backend) зацикливает стоп мёртвым токеном до рестарта агента. 🔴 Заглушка рекордера в тестах ОБЯЗАНА держать `is_recording=False` до `start()`: голый `MagicMock` даёт неявную истинность, и Core читает её как чужой захват. Спека: `docs/superpowers/specs/2026-07-25-r2-recording-ownership-design.md`. Живой смок: `scripts/e2e_owner_gate_smoke.py`.
- **`backend/recording_spill.py`** — `RecordingSpillWriter` + `finalize_part_to_wav()` (R1): continuous crash-safety spill of an in-progress recording to `<data_dir>/rescue/<uuid>.f32.part` (+ `.meta.json` sidecar), flush per chunk; disk errors fail-open (never break the recording itself, not the safety net). `finalize_part_to_wav` reassembles a `.rescued.wav` on restart.
- **`backend/recording_rescue.py`** — `run_rescue_scan()` (R1): startup scan of `rescue/*.f32.part` left by a prior (crashed) process life — finalizes to WAV, transcribes (privacy_mode-gated), pushes KrabError `audio.recording_rescued`, adds to the "Восстановленные записи" collection. Single-flight, `_MAX_PER_PASS=10`. Accepts an optional `parts=` frozen snapshot (see `service.py` — computed synchronously before the IPC socket exists, closing a race where a live `.part` from a brand-new recording could otherwise be picked up and deleted mid-write).
...
- **`backend/live_subs_service.py`** — `LiveSubsService`: streaming STT + translate for system audio subtitles. Accumulates base64 PCM 16 kHz chunks, flushes every ≥3 s or on `is_final=True`, emits `live_subs.result` via EventBus. 🔴 **Вырожденное окно (живой инцидент 2026-08-12, третий того же дня):** `is_final=True` форсировал flush БЕЗУСЛОВНО — если буфер к тому моменту уже опустошён автофлашем (или чанк пришёл пустым), снапшот получался нулевой длины и ехал прямо в STT, роняя GigaAM на WAV без фреймов (`both buffer length (0) and count (-1) must not be 0`). В логе это выглядело как вина шумоподавителя (`[STT] noise SNR=0.0 dB → denoising applied`), но 🔴 **`snr_db=0.0` — это SENTINEL «оценить не смог»**, а не измерение: ровно `0.0` возвращают `NoiseProfiler._silent_profile()` (аудио < 2048 сэмплов) и четыре ранних выхода `_compute_snr`; нормальные значения — 6–14 дБ. Плюс строка «denoising applied» печатается ДО вызова деноизера, а сам деноизер длину массива изменить не может (`_denoise_spectral_gating` выравнивает выход по `len(audio)`, а `noisereduce` в проде не установлен). Три слоя гарда: `ingest()`/`stop()` не сабмитят окно нулевой длины (is_final отвечает ПУСТЫМ результатом, а не `None` — `None` по контракту `handle_ingest` означает `reason=flush_timeout`, то есть подменил бы один ложный сигнал другим); `AudioEngine.transcribe` отсекает numpy короче `_MIN_TRANSCRIBE_SAMPLES` через `_empty_transcription_result("empty_audio", …)` (общий билдер схемы с веткой `vad_skip`); `_maybe_denoise` пропускает деноизинг при `len < _DENOISE_MIN_SAMPLES` и при `|SNR| < _SNR_UNRELIABLE_EPS_DB`, а результат прогоняет через `_guard_denoised` (пустой / сменивший форму / потерявший ≥99% энергии выход → возврат ИСХОДНОГО аудио, fail-safe в сторону «распознать шумное лучше, чем ничего»). Тесты: `tests/test_denoise_empty_audio_guard_2026_08_13.py`.
...
- **`backend/shutdown_handler.py`** — `GracefulShutdownHandler`: coordinate orderly backend shutdown (flush stores, cancel jobs, write a runtime shutdown info file). R1 (2026-07-24/25): shutdown_info.json (runtime data file, not in repo) extended with `signal`/`uptime_sec`/`recording_active`/`meeting_active`/`pid`; forensic context snapshot lives in `_capture_signal_context()` (signal-safe: no locks/I/O/logging — AST source-contract tests in `test_shutdown_info_r1_fields.py` verify this against BOTH `_capture_signal_context` and the delegating `_signal_handler`, since the sensitive body moved out from under the latter). Production's real OS signal callback is `main()`'s own closure in `service.py` (NOT `register()`), which calls `_capture_signal_context()` explicitly — `bind()` deliberately doesn't touch `signal.signal()`. `_persist()` deletes+rewrites the `runtime_alive.marker` (see `shutdown_forensics.py`) only after a *successful* write.
- **`backend/shutdown_forensics.py`** — `write_alive_marker()` / `check_and_collect()` (R1): dirty-marker (`<data_dir>/runtime_alive.marker`) written on every life, deleted only by a proven graceful `_persist()` — a marker surviving to the next start deterministically means the prior life died UNCLEAN (SIGKILL/OOM/crash), triggering a best-effort `log show`/`launchctl print`/own-log-tail collection into `<data_dir>/forensics/<ts>/` (retention: 5 newest) while the unified log still remembers. `service.py.__init__` only spawns the `startup-recovery` background thread when there's real work to do (dirty marker present OR frozen `.part` rescue candidates) — a synchronous check on every `BackendService()` construction avoids racing hundreds of unit tests' `TemporaryDirectory` teardown against an unconditional background thread that used to touch disk immediately (root cause of a red ubuntu CI run during this wave).
...
- **`main+SparkleUpdater.swift`** (2026-07-05) — автообновления Sparkle 2 (SPM). 🔴 Dev-guard: updater НЕ инициализируется, когда `.app` лежит в каталоге проекта (рядом есть `KrabEar/backend/service.py`) — иначе Sparkle переписал бы git-дерево владельца in-place; работает только для установленных копий (DMG-получатели, /Applications). 🔴 Sparkle — динамический framework: `Sparkle.framework` обязан лежать в `Contents/Frameworks/` бандла (коммитится, как parity-бинарь) и в `native/Frameworks/` для dev-бинаря (gitignored); rpath `@executable_path/../Frameworks` в Package.swift; копирование делают `build_and_deploy.command` и `scripts/assemble_signed_app.sh` (общий ассемблер, используется DMG-скриптом и `.github/workflows/release.yml`). Релиз: тег `vX.Y.Z` или workflow_dispatch → CI-green guard (krab-ear-ci) → GH Release + appcast.xml-коммит `[skip ci]`; подпись — выделенный CI-серт «Krab Ear CI Release» (секреты MACOS_CERT_P12/MACOS_CERT_PASSWORD/SPARKLE_PRIVATE_KEY). Меню «Update Channel» (stable/beta) удалено 17.09 (волна 0.5, только deletions) — вело в никуда при одном appcast; поле `Models.updateChannel` и бэкенд-ключ оставлены для payload parity.
- **`HotkeyDoubleTapDetector.swift`** — detects Right Option double-tap (300 ms window) to start Voice Assistant conversation.
- **Быстрая заметка + панель-скретчпад** (C3a 2026-07-17 + C3b 2026-07-18/19, спека `docs/superpowers/specs/2026-07-16-c3-quick-capture-design.md`): Cmd+Shift+N (настраиваемая комбинация) — запись БЕЗ вставки в активное окно, результат уходит в history + коллекцию «Быстрые заметки», opt-in дублирование в Notes/Obsidian (`main+QuickCapture.swift`). **`QuickCapturePanelController.swift`** (C3b) — плавающая NSPanel-визуализация: статус записи + mm:ss таймер, live-текст партиалов (SSE `/v1/events?filter=realtime.partial_transcript,realtime.final_transcript`, REST-процесс `:5005`, паттерн `MeetingLivePanelController`), список последних заметок (до 7, «Копировать»), кнопки старт/стоп/«Копировать всё»/«→ Notes»; visual-полировка — agy/Gemini 3.1 Pro (`docs/design-briefs/2026-07-19-quick-capture-panel-polish.md`). Точки входа живут в `main+QuickCapture.swift` (не в отдельном файле, как предполагал план — проще встроить), единственный владелец — `quickCapturePanelController` (main.swift), настройка автопоказа `quick_capture_show_panel`. 🔴 Fable-ревью ЦЕЛОГО диффа ветки (не по коммитам) нашло HIGH sibling-gate asymmetry — `setRecording(true)` был доступен ТОЛЬКО за гейтом настройки автопоказа (вручную открытая панель при дефолтной выключенной настройке никогда не отражала собственный старт записи из своей же кнопки), а `setRecording(false)` на стопе был гейтен `isVisible` (закрыть панель мид-записи навсегда замораживало внутренний `isRecording=true` — guard в `setRecording` блокирует все будущие ресинхронизации). Оба гейта сняты — состояние панели теперь безусловный источник правды; `resyncTimerAndPulseIfNeeded()` восстанавливает таймер/пульс при повторном `show()`. Тот же паттерн, что нашёл 3 HIGH в C3a — целостный ревью диффа ловит то, что пропускают и построчный self-гейт, и per-task воркеры.
...
- **`HealthMonitor.swift`** — actor с 3s ping (IPC-метод ровно `"ping"`, НЕ `handle_ping` — так зовётся лишь внутренний Python-хендлер); 2 fails подряд → `state = .hung` → **однократный** `onHangDetected` → `BackendSupervisor.restartIfDeadDetailed()`. Сигналы отсюда НЕ шлются (раньше эта строка описывала несуществующий SIGTERM→SIGKILL→respawn и в разборе инцидента 2026-08-07 увела диагностику не туда). Вторая ступень заклинивания ЗАКРЫТА: `setWedgeProbe` + `setOnWedgeDetected` в `HealthMonitor.swift` / `main+HealthMonitor.swift` — `forceRestartBackend()` под `WedgedEscalationTracker`, только если соединение ОТВЕРГАЕТСЯ (`IPCError.socketConnectFailed`), процесс старше 600 с, был хотя бы один живой ping, и нет записи/встречи. `IPCError.timeout` намеренно НЕ эскалация (медленный backend ≠ заклинивший). Тесты: `HealthMonitorWedgeTests.swift`. Оставшийся класс «accept жив, хендлеры стоят» — не этот сторож; см. спеку `docs/superpowers/specs/2026-08-16-w2-daily-stability-design.md` §0.1. Phase B.1: подписка на `rewriter_recovered` через EventBridge живая.
...
- **Single-instance guard** (hardening 2026-07-20): единственная автоматическая гарантия — POSIX `flock`; startup больше не ищет и не завершает процессы по имени или PID. Даже двойная проверка `proc_pidpath`/start-time не закрывает TOCTOU между последним чтением identity и `kill(pid, ...)`, потому на macOS без атомарного process handle legacy-cleanup удалён целиком. Старые экземпляры обслуживаются только вручную. Source-контракт запрещает возврат `pgrep`/`killOtherAgentInstances`/`SIGKILL`/`SIGTERM`/`kill(` в `SingleInstanceGuard.swift` и startup `main.swift`. Тесты `LaunchAgentManagerTests` и `PermissionWizardTests` обязаны использовать injected UUID-temp `LaunchAgents` directory + process-runner/`AutostartManaging` spy; реальный `~/Library/LaunchAgents` и `/bin/launchctl` в unit-тестах запрещены.
...
- **🔴 Аварийные сигналы — только `faulthandler`, никогда Python-обработчик (инцидент 2026-08-07)**: вешать `signal.signal()` на СИНХРОННЫЙ сигнал (SIGSEGV/SIGBUS/SIGFPE/SIGILL/SIGABRT) запрещено. CPython не исполняет колбэк в момент сбоя — C-уровень ставит флаг и ВОЗВРАЩАЕТСЯ, ядро повторяет сбойную инструкцию, сегфолт зацикливается навсегда. `install_signal_handlers()` делал именно это (слал событие в Sentry перед смертью) и в проде дал: рабочий поток 1902/1966 сэмплов в `_sigtramp`, 46 вложенных Python-обработчиков на главном потоке, дедлок на локе внутри `sentry_sdk.flush()` — процесс не принимал IPC, не умирал и НЕ мог обработать SIGTERM (лечил только SIGKILL), т.е. честный крэш с автоперезапуском launchd превращался в многочасовой ТИХИЙ простой. Теперь — `faulthandler.enable(all_threads=True)` (signal-safe C-уровень: трейсбек ВСЕХ потоков в stderr → err.log + передача сигнала default-обработчику). `all_threads` обязателен: сбой прилетает в рабочий поток. Тот же запрет — на любую не-signal-safe работу (лок/IO/логирование/сторонняя библиотека) в ЛЮБОМ обработчике: он обязан только выставить флаг (см. `service.main()._signal_handler` и `IPCServer.request_stop_from_signal`). Гейт: `tests/test_fault_signal_handler_2026_08_07.py` (поведение + AST-контракт на отсутствие `signal.signal`/`raise_signal`), sample живого процесса — `.remember/forensics/`.
...
- **Wave 68 (PR #415) — отменена 2026-08-26, но её замер был ОШИБОЧНЫМ (поправка 03.09.2026)**: перевод `_handle_list_llm_models` на `/api/v1/models` тогда сочли причиной пустого списка. Повторный замер на живом LM Studio 0.4.x показал: **все три endpoint'а отдают 105 моделей**, а «200 с пустым телом» был ответом **401 без Bearer-токена** — LM Studio с включённой аутентификацией отвечает 401 на любой запрос каталога, и тело ошибки читалось как пустой каталог. 🔴 Настоящая причина слепоты: **три формы одного ответа**, идентификатор в разных ключах — `/api/v0/models` → `{"data":[{"id","state"}]}`, `/api/v1/models` → `{"models":[{"key","loaded_instances"}]}`, `/v1/models` → `{"data":[{"id"}]}`. Парсер `data[].id` не понимал нативный v1 и отвечал «моделей нет» вместо «не разобрал». Отсюда вечные `has_model=false` в `probe_llm_http` и `None` в `model_loaded`. Единый разбор — `lm_studio_lifecycle.parse_lm_studio_catalog` (#1984): возврат трёхзначный (`None` — форму не распознал, `[]` — каталог пуст), `loaded` тоже трёхзначное. Проверка «загружена?» опрашивает только `LOADED_STATE_ENDPOINTS` (v0/v1) — OpenAI-совместимый список состояния не несёт, и любая скачанная модель выглядела бы загруженной. 🔴 Фильтровать по `type == llm` нельзя: мультимодальные (`vlm`) модели пригодны для рерайта. Исключаются только `embeddings`.
...
- **B3 brain-lease видимость (2026-07-19)**: IPC `get_brain_lease_status {}` → `{ok, enabled, held, owner|null, pid|null, acquired_ts|null, exp_ts|null, seconds_left|null}` (`HealthCheckService.handle_get_brain_lease_status`; общий билдер `_build_brain_lease_summary()` даёт и секцию `brain_lease` в `get_diagnostics`; payload lock-файла пишет ЧУЖОЙ процесс — типы коэрсятся с fallback null, схеме не доверяем). Swift: `main+BrainLease.swift` — disabled-строка «LM Studio: свободен / Krab Ear · ещё Nс / Краб · ещё Nс» в status-меню сразу после recap-карточки, обновляется в `menuWillOpen` (паттерн `refreshQuickNotesSubmenu`, БЕЗ фонового поллинга); скрыта при `llm_brain_lease_enabled=false`, при IPC-провале — «LM Studio: —» (не скрывать: скрытие маскировало бы мёртвый backend). Счётчик хендлеров: **360**.
- **S34 clipboard safety (2026-07-19)**: `PasteService.putToClipboard(_:) -> Bool` (`@discardableResult`) — единая точка защиты — не затирает системный буфер, если он помечен `org.nspasteboard.ConcealedType` (де-факто стандарт nspasteboard.org, менеджеры паролей 1Password/Bitwarden/Keychain Access так помечают чувствительный контент); Bool-результат ОБЯЗАН проверяться вызывающей стороной. **Fable-ревью нашло HIGH до мержа (F1)**: `pasteToFrontmostApp` игнорировал void-версию и безусловно слал синтетический Cmd+V — при пропущенной записи это вставляло СТАРОЕ (защищённое) содержимое буфера в frontmost app, то есть фикс был строго хуже исходного бага (потеря пароля → раскрытие пароля). Исправлено: `guard putToClipboard(...) else { return PasteAttemptResult(reason: "concealed_clipboard_skipped") }` ДО любых key events; амплификация через `StreamingPasteController.appendChunk` (per-chunk) закрыта тем же фиксом (адаптер тонкий, делегирует напрямую). `handlePasteFailure` получил тихую раннюю ветку по этой reason (паттерн `secure_field_skipped`) — БЕЗ повторного `putToClipboard`. Explicit user-initiated copy (`onCopyLastResult`, `onQuickNoteItemClicked`, QuickReplace) переведены на отдельный `putToClipboardUserInitiated(_:)` — безусловная запись, минуя guard (F3 — иначе явные «Копировать»-действия тихо переставали работать при защищённом буфере, вразрез со спекой). Все notify()-сообщения по цепочке (`continueTranscriptionResult`/`handlePasteFailure`) теперь отражают реальный Bool-результат записи, а не безусловное "скопировано" (F2). `onConcealedClipboardSkipped` closure — только для логирования (НЕ notify — иначе дублировало бы уведомление из call site/`handlePasteFailure`). Risk-warning tooltip у `clipboardModeSelector` (`HistoryPanelController.swift`) и у пунктов подменю «Буфер обмена» (`main+StatusMenu.swift`). Осознанно НЕ save/restore-таймер (сломал бы фичу «текст ждёт в буфере до ручной вставки») — только guard перед перезаписью для транспортных путей, безусловная запись для explicit-copy.
...
`scripts/status_dashboard.py` — живое состояние экосистемы: прод-процессы,
загрузка машины, движки и задания воркеров ПО ТРЁМ ПРОЕКТАМ, открытые PR,
смерженное за сутки, живые сессии Claude.

```bash
python3 scripts/status_dashboard.py            # текст в терминал
python3 scripts/status_dashboard.py --json     # машинный вид
python3 scripts/status_dashboard.py --serve 8777
```

Постоянная служба: LaunchAgent `ai.krab.ear.dashboard` (RunAtLoad + KeepAlive),
`http://127.0.0.1:8777`. 🔴 Сервер, а не статический файл: данные пересобираются
на КАЖДЫЙ запрос, страница сама обновляется — снимок требовал ручной пересборки
и владелец сразу упёрся в это. Порт **8080 занят панелью главного Краба** из
его release-worktree; трогать оттуда нельзя, ссылка добавляется их стороной.

Воркеры читаются из `<проект>/.remember/tmp/workers/out_*.log`; завершение —
строка `DONE rc=<код>`. 🔴 Класть логи воркеров именно туда, а НЕ в `/tmp`:
перезагрузка машины 02.09 стёрла шесть непрогейченных отчётов. И «ноль байт» ≠
смерть — `cursor-agent` буферизует вывод до конца, поэтому «работает»
определяется по отсутствию маркера `DONE`, а не по размеру файла.

### 🔴 Самоблокировка через передачу работы в пул потоков (класс, 02.09.2026)

Держатель берёт `mlx_lock` в своём потоке и внутри удержания отдаёт работу в
`ThreadPoolExecutor`; рабочий поток берёт ТОТ ЖЕ лок и ждёт того, кто его
породил. `RLock` реентерабелен ТОЛЬКО для своего потока — передача в пул это
ломает. Симптом в проде: `GigaAM-MLX: mlx_lock занят дольше 25.0с` на каждой
итерации превью, «preview worker не завершился за 1.5 с», диктовка 90с на 30с
речи и «Все доступные STT-движки вышли из строя».

Оба известных места закрыты: превью (`transcriber.transcribe_preview` — лок
отпускается сразу после пробы занятости, PR #1972: 1с вместо 90с на живом
замере) и `bulk_reprocess.py` (внешний захват снят целиком, PR #1974). Третьего
сиблинга нет: прочие держатели `mlx_lock` (`audio_lang_id`, адаптеры,
`_transcribe_model`, `scripts/debug_whisper.py`) зовут MLX сами, без делегации.

Снятие в bulk потребовало пройти ВСЕ MLX-пути внутри `engine.transcribe` и
доказать, что каждый берёт лок сам, в своём потоке: `_transcribe_model` (whisper
+ RU-finetune, оба `submit`), GigaAM-MLX `_infer_chunk`, parakeet, voxtral,
`AudioLanguageID._run_detect`, `set_quality_profile`, пост-STT `mx.clear_cache()`;
пути мимо адаптеров и мимо `_transcribe_model` (`_transcribe_remote` — сеть,
диаризация — PyTorch) MLX не трогают.

🔴 **Межпроцессный flock самоблокируется так же** (замерено): другой поток того
же процесса, открыв НОВЫЙ fd на тот же файл, не получает `LOCK_EX` — flock
привязан к описанию открытого файла, не к процессу.

Правило: захват лока перед вызовом, который уходит в другой поток, — всегда
подозрителен. Регресс-тест обязан проверять захват ИЗ ДРУГОГО ПОТОКА: для
своего потока RLock реентерабелен, и баг невидим. Гард на такой инвариант пишем
разбором AST, а не подстрокой: честный комментарий про `mlx_lock` в исправленном
файле роняет подстрочную проверку (напоролись при PR #1974).
