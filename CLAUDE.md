...
Починено для превью (`transcriber.transcribe_preview` — лок отпускается сразу
после пробы занятости, PR #1972: 1с вместо 90с на живом замере).
🔴 **Сиблинг ОСТАЁТСЯ**: `bulk_reprocess.py` держит оба лока через весь
`transcribe()`; снятие блокирует тест-инвариант W1037 F2 и требует доказать,
что каждый путь берёт лок сам — вынесено отдельной задачей.
...
своего потока RLock реентерабелен, и баг невидим.
