...
- **Wave 68 (PR #415) — 🔴 ОТМЕНЕНА 2026-08-26 как вредная**: перевод `_handle_list_llm_models` на `/api/v1/models` не «устранил тихий пустой список», а СОЗДАЛ его. Замер на живом LM Studio 0.4.21 (105 моделей на диске): `/api/v0/models` → 200/105, `/v1/models` → 200/105, **`/api/v1/models` → 200/0**. Endpoint отвечает 200 с пустым телом, поэтому IPC годами возвращал `{models: [], error: null}` — ошибки нет, данных нет, GUI показывал только захардкоженный в Swift список. Сейчас каталог берётся из `/api/v0/models` (даёт `type`/`state`/`quantization`) с фоллбэком на `/v1/models`; пустой каталог трактуется как ОШИБКА, а не как «моделей нет». 🔴 Фильтровать по `type == llm` нельзя: мультимодальные (`vlm`) модели пригодны для рерайта — у `gemma-4-26b-a4b-it@4bit` тип именно `vlm`. Исключаются только `embeddings`.
