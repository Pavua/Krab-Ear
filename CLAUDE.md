...
- **Sharing/archive path containment (wave-33, #1737)**: File operations using attacker-controlled fields from persisted JSON (e.g., `shares_index.json` `filename` key) MUST use `resolved_path.is_relative_to(base_dir)` before unlink/rmtree. A planted `filename='../../x'` enables arbitrary file deletion.
