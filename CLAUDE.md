...
- **B3 brain-lease видимость (2026-07-19)**: IPC `get_brain_lease_status {}` → `{ok, enabled, held, owner|null, pid|null, acquired_ts|null, exp_ts|null, seconds_left|null}` (`HealthCheckService.handle_get_brain_lease_status`; общий билдер `_build_brain_lease_summary()` даёт и секцию `brain_lease` в `get_diagnostics`; payload lock-файла пишет ЧУЖОЙ процесс — типы коэрсятся с fallback null, схеме не доверяем). Swift: `main+BrainLease.swift` — disabled-строка «LM Studio: свободен / Krab Ear · ещё Nс / Краб · ещё Nс» в status-меню сразу после recap-карточки, обновляется в `menuWillOpen` (паттерн `refreshQuickNotesSubmenu`, БЕЗ фонового поллинга); скрыта при `llm_brain_lease_enabled=false`, при IPC-провале — «LM Studio: —» (не скрывать: скрытие маскировало бы мёртвый backend). Счётчик хендлеров: **365**.
