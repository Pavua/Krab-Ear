...
- ~~Экспорт сертификата «Krab Ear Dev Local»~~ **Изменено при имплементации (2026-07-05, безопаснее)**: вместо экспорта локального ключа владельца сгенерирован ВЫДЕЛЕННЫЙ CI-сертификат **«Krab Ear CI Release»** (openssl, полностью в файлах, keychain владельца не тронут) → GitHub Secrets: `MACOS_CERT_P12` (base64), `MACOS_CERT_PASSWORD`. Обоснование: TCC-непрерывность владельца от CI-серта не зависит вовсе (dev-guard исключает его Mac из Sparkle-пути; локальные сборки подписывает локальный серт), а получатели DMG сегодня получают ad-hoc подпись — им смена серта при первом Sparkle-релизе неизбежна в любом случае. Релизы между собой подписаны одним CI-сертом → TCC-непрерывность между обновлениями у получателей сохраняется. Ключи владельца не покидают его машину.
