...
шина IPC-процесса; живая проверка — Шаг 0 плана).
...
Живой проверкой зафиксировать текущее состояние (ожидание: путь мёртв):
...
3. Результат (событие пришло/нет) — в план волны как факт. Если ВНЕЗАПНО живо — остановиться
   и разобраться (значит, модель двух шин неверна и дизайн пересматривается).
