# A1 — Рекомендованная настройка в один тап — ЧЕРНОВИК (инвентаризация)

> ## СТАТУС: ЗАКРЫТ 2026-07-07 — все 7 вопросов §9 решены владельцем/контролёром.
> Финальная спека: `docs/superpowers/specs/2026-07-07-recommended-setup-design.md` (решения + итоговый
> контракт). **Этот файл остаётся источником истины по инвентаризации** (таблица 39 кандидатов,
> §1-§4, §8) — финальная спека его не дублирует, только резюмирует и ссылается сюда. Вердикты в
> таблице §4 (ДА/УСЛОВНО-ДА/ВОПРОС/НЕТ/МЁРТВЫЙ) остаются авторитетными кроме одного явного
> переопределения: GigaAM-пара (`stt_gigaam_enabled`+`stt_language_routing_enabled`) была
> УСЛОВНО-ДА здесь (с probe венва), но контролёр решил (9.7) — ВСЕГДА `skip`, без probe-логики.
