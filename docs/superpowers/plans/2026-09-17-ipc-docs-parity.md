...
- `CLAUDE.md`: строка «handler count now **360**» (2026-07-01, с методикой — оставить как историю) и «Счётчик хендлеров: **365**» (2026-07-19, B3 — живой счётчик, обновить на 360). 🔴 Строка «handler count now **359**» (2026-06-27) — тоже история, НЕ переписывать. Проверено `rg`, перепроверить в Task 3.
...
- [ ] **Step 1: Для каждого missing-ключа** добавить строку в категориальную таблицу + секцию `### \`метод\``. Маршрутизацию брать из значения dispatch (`"m": self._history.handle_x` → `(service.py → history_service.py)`); модуль сверить: `def handle_x` обязан существовать в файле модуля. Особый кейс: собственные методы сервиса (`"purge_all_data": self._handle_purge_all_data`, `service.py:2880`) — маршрутизация `(service.py)`, без стрелки. Категорию выбирать по соседству: секция, где уже лежат методы того же `self._<сервис>.`; если такой нет — ближайшая по смыслу + пометка в отчёте. Описание — 1 строка по сигнатуре хендлера (params/returns из кода, не выдумывать).
...
- [ ] **Step 3: `CLAUDE.md`** — заменить `**365**` на `**360**` ТОЛЬКО в строке B3 brain-lease (2026-07-19, живой счётчик); строки 2026-06-27 (`**359**`) и 2026-07-01 (`**360**`) — датированная история, не трогать. Затем `rg -n '\*\*(359|360|365)\*\*' CLAUDE.md` — обязаны остаться ровно эти три, причём B3 — 360.
