...
### Task 2: прогрев (warmup-путь) — ВЕРДИКТ 15.09: исходников не требует

Расследование показало, что чинить нечего:
- Роутинг GigaAM-first для RU уже реализован (`engine.py:2696-2714`)
  и покрыт тестами (`test_gigaam_used_first_for_ru` + 3 файла).
  Прогретая диктовка #4 (14.09) целиком прошла через GigaAM-MLX.
- Warmup существует (`_warmup`: silence, раз на процесс) и покрыт
  (`test_stt_gigaam_mlx_adapter.py`). Cold-loss 20:50:33 — средовой
  (холодный Metal + vm_pressure), не логический разрыв; на demand
  не воспроизводится, гарантию кодом не закрыть без спекуляций.
- Приёмка (non-empty → accept; loss → ChunkLoss → каскад) — осознанный
  дизайн с fallback-безопасностью. Менять acceptance (качество по
  conf-константе) — решение уровня #1985, не этой волны.
Итог: T2/T3 без source-правок. Ценность волны: T1-harness как
regression-lock + этот вердикт. Мерж ветки = тест + карточка.

### Task 2 (архив, не исполнять): прогрев (warmup-путь)
...
### Task 3 (архив, не исполнять): роутинг финалов — не требуется, см. вердикт в T2
