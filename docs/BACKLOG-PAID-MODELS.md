...
Контекст 2026-09-14→17: основная сессия идёт на бесплатной
**Muse Spark 1.3 Contributor** (Zen contributor-tier — промпты и ответы
используются для обучения будущих моделей Meta). Владелец разрешил данные
своих диктовок/архива для майнинга лексики (16.09), но правило прежнее:
сырые тексты в контекст не тащить — только агрегаты локальным скриптом
(`/tmp/mine_vocab.py`).

Статус Ф0 на 17.09: пп. 1–4 ВЫПОЛНЕНЫ (small_model, агенты executor +
gate-security файлами, MCP sentry/context7/chrome/hammerspoon, Sentry-токен
в shell через деривацию) — всё ждёт только рестарта opencode (не квоты!).
Остаток: п. 5 (рестарт + проверка) + платные гейты/исполнение по квоте.

## Ф0-остаток (проводка opencode)

1. `opencode.json`: `small_model` → `opencode-go/glm-5.3-flash`.
2. Агент `executor` (subagent, `opencode-go/deepseek-v4.1-flash`,
   playbook-дисциплина в промпте; акция до 20.09; ID подтверждён скриншотом
   каталога владельца 14.09 — публичный `/zen/v1/models` его не отдаёт,
   валидировать при проводке; fallback `deepseek-v4-flash-free`) + агент
...
## Очередь исполнителя (DeepSeek V4.1 Flash, акция до 20.09)

- R2, R4, W1, W2 — DONE на free-сессии (в колее).
- C6 HealthMonitor `timeoutSec: 2`.
- Поддержка agy-брифа «Глоссарий» (тексты, не визуал).
- W4 STT-лексика: майнинг готов, ЖДЁТ аппрува слов владельцем.
