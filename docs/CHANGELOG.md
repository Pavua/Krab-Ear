...
Финальный хардинг и расширение покрытия. 41 коммит поверх `codex/krab-ear-v2`.
282 файла изменено, +77 013 строк. 168 тест-файлов, **4099 тестов (0 ошибок)**, 152 Python-модуля, 90 961 строк.
...
#### Аудио / STT
- **Gain Normalizer** — нормализация уровня входного сигнала (32 теста)
- **Word Timing** — извлечение временных меток слов из Whisper-транскрипта
- **Smart Model Selector** — автоматический выбор модели STT по параметрам задачи
- **Playback Tracker** — отслеживание воспроизведения/прослушивания записей
- **Recording Insights** — метрики качества записи: SNR, паузы, темп (39 тестов)

#### История и хранилище
- **Archive Manager** — архивация и разархивация старых записей (31 тест)
- **Search History** — журнал поисковых запросов с топ-N (22 теста)
- **Activity Calendar** — тепловая карта активности транскрипций по дням (37 тестов)
- **Stats Report Generator** — детальный отчёт по статистике использования (51 тест)
- **Auto-deduplication** — автоматическое обнаружение и слияние дублей (29 тестов)
- **Timeline Export** — экспорт временной шкалы сессии (31 тест)

#### Аналитика и мониторинг
- **Health Dashboard** — полная сводка состояния системы: память, CPU, сокет, модели (51 тест)
- **Metadata Enricher** — автоматическое обогащение метаданных записей (65 тестов)
- **Comparison Module** — сравнение пар транскриптов/сессий (29 тестов)
- **Export Scheduler** — автоматический экспорт по расписанию (40 тестов)
- **Smart Vocabulary Suggestions** — умные подсказки словаря из контекста (22 теста)

#### Текстовая обработка
- **Text Post-Processor** — финальный пост-процессинг транскрипта (58 тестов)

#### Инфраструктура
- **Graceful Shutdown** — корректное завершение всех сервисов по SIGTERM
- **Startup Diagnostics** — самопроверка при запуске: зависимости, модели, сокет

#### Тесты
- **4099 тестов пройдено** (4 пропущено), 0 ошибок
- **168 тест-файлов** (+20 по сравнению с v2.1.0)
- Новые наборы: health dashboard (51), metadata enricher (65), stats report (51),
  text post-processor (58), export scheduler (40), activity calendar (37),
  archive manager (31), timeline export (31), recording insights (39),
  auto-dedup (29), comparison (29), smart vocab (22), gain normalizer (32)
