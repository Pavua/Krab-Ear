...
2. В его стеке — **общий C-обработчик CPython** `signal_handler` → `trip_signal`.
   CPython ставит этот обработчик ТОЛЬКО для сигналов, которым назначен
   Python-колбэк. SIGBUS/SIGFPE/SIGILL оставались на `SIG_DFL` и просто убили бы
   процесс. Значит шторм-сигнал мог быть только **SIGSEGV или SIGABRT** — ровно
   те два, что регистрировал `install_signal_handlers()`.
3. Главный поток — 46 вложенных Python-обработчиков: каждый следующий сбой
