...
3. Живой e2e на M4 Max — без запуска второго прод-инстанса (прямые вызовы
   ...
   **Фактический результат e2e 31.07** (полный лог — в PR): (а) ЧАСТИЧНО —
   RNNT-MLX 8.6× RT против голого cpu-RNNT 3.2× RT = ускорение 2.7×
   (порог 5× не достигнут: cpu на M4 Max оказался быстрее заложенной в
   порог оценки; замер cpu — без IPC-накладных, т.е. консервативен);
   CTC-MLX — 40× RT (~12× к cpu) — превышает порог, кандидат для коротких
   войсов. Subprocess end-to-end не замерялся: singleton guard (Wave 525)
   прод-воркера — намеренная защита, прод не трогали. (б) ВЫПОЛНЕНО:
   оба транскрипта корректны, диаризация — community-1 (W-B live),
   конвейер ~2× RT. (в) подтверждено юнит-гейтами (262 теста, флаги
   default False). Решение о включении флагов в проде — за владельцем.
