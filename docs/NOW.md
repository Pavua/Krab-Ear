...
🔴 **Не закрыто, зафиксировано:** параметр `_empty_transcription_result(engine=...)` фактически несёт ПРИЧИНУ (`empty_audio`/`vad_skip`), а не имя движка — поле `engine` в REST-ответе врёт. Переименование рискованно для читателей поля, поэтому #1937 завёл рядом честное поле, а смысловой баг остался. Аварийный механизм `OomAutoRelief` — боевой путь НЕ пройден (ждёт первого настоящего OOM), это [[reference_emergency_mechanism_zero_runs_means_untested]].
