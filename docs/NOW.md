...
🔴 **НАЙДЕНО 2026-08-18 21:00, НЕ ПОЧИНЕНО: мост событий IPC→REST отбивается 401, события не доходят.** Вскрыто попутно при сверке 504 с Voice Gateway. После рестарта 20:44 на `/internal/event` **0 успехов против 782 отказов** (исторически успехов 302 430 — то есть мост работал). Отправитель — наш backend (`EventBridge запущен` в 20:43:57; Swift-агент на этот эндпоинт не шлёт вовсе).

Два отдельных дефекта. (1) **Диагностика врёт**: `get_diagnostics.event_bridge` показывает `sent=0, failed=0, queue_depth=0, state=unknown` — HTTP 401 не считается ни успехом (не 2xx), ни провалом (не исключение), поэтому непрерывно отбиваемый мост выглядит как «ничего не происходит». Класс [[feedback_always_red_monitor_is_blind]] наоборот: всегда-зелёный монитор. (2) **Причина 401 не в data-dir** (проверено: `KRAB_EAR_DATA_DIR` одинаков у обоих юнитов, файл токена ровно один) и не в отсутствии re-read (он есть: `rest_server.py` сбрасывает кэш и перечитывает файл). Рабочая гипотеза: backend держит в памяти токен, которого НЕТ в файле — `_load_or_create_token` создал новый, но запись на диск не состоялась, `mtime` файла остался **8 июля**. Тогда 401 вечен по построению.

🟡 **УТОЧНЕНО в 21:22 — мост САМОВОССТАНОВИЛСЯ**, вывод «мост мёртв» был преждевременным. Хронология: отказы шли пачками с 20:44 до 21:13 (~1600 штук), в 21:15:26 прошла первая успешная доставка, после неё ни одного 401. Диагностика сейчас: `state=up, sent=1`. То есть re-read в итоге сработал, но окно потери событий составило **~30 минут после рестарта**.

Что остаётся настоящей находкой (не самоизлечилось): **`failed=0` при ~1600 реальных отказах**. Пара счётчиков `sent`/`failed` не покрывает третий исход — HTTP-ответ ≥400 приезжает без исключения и без 2xx, поэтому не инкрементирует ничего. Признак вранья счётчиков: `sent + failed` не сходится с числом попыток. Второе, что остаётся: 30-минутное окно потери SSE-событий после каждого dual kickstart — живые панели встречи и быстрой заметки в это время не получают партиалы. Диктовка и хоткеи не затронуты. Волна не начата — решение владельца.
