...
6. **W4 STT-лексика** — майнинг готов (23k диктовок + 192k сегментов архива,
   ...
   ПРЕДУТВЕРЖДЕНО владельцем 17.09 (`lrd`=@p0lrd, `висперед`=whisper,
   `maby`~«могу» — только с RU-гейтом) → ре-майнинг топ-листа + запекание в F1.
7. **Волна 0 (гигиена, до 30.09)** — 0.1 (изоляция e2e-моста) и 0.2 (контракт)
   DONE+смержены 17.09; в работе: 0.3 (quality_profile из настроек), 0.4
   (паритет IPC-документации), 0.5 (убрать меню Update Channel).
