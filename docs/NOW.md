...
**Открыто владельцу:** мёртвая цепь `auto_cleanup_*` (достроить или удалить);
односторонний пакет «рекомендованная настройка» (7 ключей включаются, не выключаются);
Swift-тесты не гоняются в CI вовсе (кандидат: `--filter "WiringTests|SourceContract"`, 1.1 с).
